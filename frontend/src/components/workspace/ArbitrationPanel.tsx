...
const VERDICT_STYLES: Record<string, { bg: string; text: string; border: string; label: string }> = {
  ...
  acknowledged: { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200', label: 'Acknowledged' },
  must_fix: { bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-200', label: 'Must Fix' },
  deferred: { bg: 'bg-yellow-50', text: 'text-yellow-600', border: 'border-yellow-200', label: 'Deferred' },
  needs_human: { bg: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-200', label: 'Needs Human' },
};
      ...
      <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-amber-700">Arbitrator 裁决</span>
          </div>
                  ...
                  {critical ? (
                    ...
                    <>
                      <button
                        ...
                        className="px-2 py-1 text-xs bg-orange-50 text-orange-600 border border-orange-200 rounded hover:bg-orange-100 transition-colors"
                      >
                        Upgrade
                      </button>
                      <button
      ...
      <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-end gap-3">
        {mode === 'view' ? (
          <>
            ...
            <button
              onClick={onAccept}
              className="px-4 py-2 text-xs font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors"
            >
              接受全部裁决
            </button>
          </>
