          ...
          <div key={round}>
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-800/80 rounded-full">
                  ...
                  {(acceptCount > 0 || rebutCount > 0) && (
                    <span className="text-xs text-gray-500">
                      ...
                      {acceptCount > 0 && rebutCount > 0 && ' · '}
                      {rebutCount > 0 && <span className="text-yellow-400">{rebutCount} 反驳</span>}
                    </span>
                  )}
                  {satisfiedCount > 0 && (
                    <span className="text-xs text-green-400">{satisfiedCount}/3 满意</span>
                  )}
                </div>
                <div className="h-px flex-1 bg-gradient-to-r from-transparent via-gray-700 to-transparent" />
              </div>
            ...
            <div className="space-y-3">
              ...
              {crossMsgs.length > 0 && (
                <div className="bg-purple-900/10 border border-purple-800/30 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-purple-400">交叉审阅</span>
                    <span className="text-xs text-purple-500/60">Attacker 互相补充观点</span>
                  </div>
              ...
              {arbitratorMsgs.map((msg, i) => (
                <div key={`arb-${round}-${i}`} className="border border-amber-800/40 bg-amber-900/10 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
              ...
              {/* Judge messages */}
              {judgeMsgs.map((msg, i) => (
                <div key={`judge-${round}-${i}`} className="border border-purple-800/40 bg-purple-900/10 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
...
function CoderResponseCard({ message, round }: { message: DebateMessage; round: number }) {
  ...
  return (
    <div className={`rounded-lg border ${isFix ? 'border-amber-800/40 bg-amber-900/5' : 'border-blue-800/40 bg-blue-900/5'} p-4`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`w-6 h-6 rounded-full ${isFix ? 'bg-amber-600/20' : 'bg-blue-600/20'} flex items-center justify-center`}>
            <svg className={`w-3.5 h-3.5 ${isFix ? 'text-amber-400' : 'text-blue-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
            </svg>
          </div>
          <span className={`text-sm font-semibold ${isFix ? 'text-amber-400' : 'text-blue-400'}`}>
            ...
            return (
              <div
                ...
                className={`rounded-lg px-3 py-2 ${
                  isAccept ? 'bg-green-900/15 border border-green-800/30' : 'bg-yellow-900/15 border border-yellow-800/30'
                }`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm">{isAccept ? '✅' : '❌'}</span>
                  <span className={`text-xs font-semibold ${isAccept ? 'text-green-300' : 'text-yellow-300'}`}>
        ...
        <div className="space-y-1.5">
          {findings.map((f, i) => (
            <div key={i} className="bg-gray-50 rounded px-2.5 py-1.5">
              ...
              {f.test_input && (
                <div className="mt-1 px-2 py-1 bg-gray-900/60 rounded text-xs text-gray-500 font-mono truncate">
                  test: {f.test_input}
                </div>
              )}
            </div>
