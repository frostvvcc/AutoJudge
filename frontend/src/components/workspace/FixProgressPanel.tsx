...
const STATUS_STYLES: Record<AttemptStatus, { bg: string; text: string; border: string; label: string }> = {
  pending: { bg: 'bg-gray-800/60', text: 'text-gray-500', border: 'border-gray-700/40', label: 'Pending' },
      ...
      <div className="px-4 py-3 border-b border-gray-800 bg-gray-900/80">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-300">Fix Progress</span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-2 px-2.5 py-1.5 rounded ${
                    isResolved ? 'bg-green-900/10 border border-green-800/20' : 'bg-gray-800/30'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-gray-500 bg-gray-800/50 px-1 py-0.5 rounded">{item.id}</span>
                  ...
                  <div className={`flex-1 mb-3 rounded-lg border p-3 transition-colors ${style.border} ${style.bg}`}>
                    ...
                    {isExpanded && (
                      <div className="mt-2 pt-2 border-t border-gray-700/30">
                        <pre className="text-xs text-gray-400 whitespace-pre-wrap leading-relaxed font-mono bg-gray-800/30 rounded p-2 max-h-[200px] overflow-y-auto">
                          {attempt.content}
                        </pre>
                      </div>
                    )}
